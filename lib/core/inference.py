...
def get_multi_stage_outputs(
        cfg, sess, tensor_output, tensor_image, image, with_flip=False,
    ...
    for i, output in enumerate(outputs):
        if len(outputs) > 1 and i != len(outputs) - 1:
            output = cv2.resize(
                output[0],
                dsize=(outputs[-1].shape[2], outputs[-1].shape[1]),
    ...
    if with_flip:
        ...
        heatmaps_avg = 0
        num_heatmaps = 0
        outputs_flip = sess.run(
            tensor_output,
            feed_dict={tensor_image: [image[:,::-1,:]]}
            )
        for i in range(len(outputs_flip)):
            ...
            if len(outputs_flip) > 1 and i != len(outputs_flip) - 1:
                output = cv2.resize(
                    output[0],
                    dsize=(outputs_flip[-1].shape[2], outputs_flip[-1].shape[1]),
                    interpolation=cv2.INTER_LINEAR
                )[np.newaxis,:,:,:]
            output = output[:,:,::-1,:]
    ...
    if project2image and size_projected:
        heatmaps = [
            cv2.resize(
                hms[0],
                dsize=(size_projected[1], size_projected[0]),
                interpolation=cv2.INTER_LINEAR
            )[np.newaxis,:,:,:]
            for hms in heatmaps
        ]
    ...
    if scale_factor == 1 or len(cfg.TEST.SCALE_FACTOR) == 1:
        if final_heatmaps is not None and not cfg.TEST.PROJECT2IMAGE:
            tags = [
                cv2.resize(
                    tms[0],
                    dsize=(final_heatmaps.shape[2], final_heatmaps.shape[1]),
                    interpolation=cv2.INTER_LINEAR
                )[np.newaxis,:,:,:]
                for tms in tags
            ]
        for tms in tags:
            tags_list.append(tms[:,:,:,:,np.newaxis])
