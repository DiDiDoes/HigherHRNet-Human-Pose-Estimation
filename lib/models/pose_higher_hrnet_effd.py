...
class EfficientDetHigherResolutionNet(nn.Module):

    def __init__(self, cfg, **kwargs):
        self.inplanes = 64
        extra = cfg.MODEL.EXTRA
        super(EfficientDetHigherResolutionNet, self).__init__()

        self.backbone_neck = EfficientDetBackbone(compound_coef=0)
        
        fpn_filters = self.backbone_neck.fpn_filters()
        self.final_layers = self._make_final_layers(cfg, fpn_filters)
        self.deconv_layers = self._make_deconv_layers(
            cfg, fpn_filters)
...
def get_pose_net(cfg, is_train, **kwargs):
    model = EfficientDetHigherResolutionNet(cfg, **kwargs)
